...
import type { RuntimeCommand, RuntimeCommandType, CommandSource } from './contracts'
...
export abstract class BaseRuntimeInputAdapter implements RuntimeInputAdapter {
  ...
  protected emitCommand(
    ...
    if (event.type === 'runtime:command-executed') {
      this._commandsEmitted++
      this._lastSuccessTime = Date.now()
    } else {
      this._commandsDropped++
      this._lastError = (event.payload as { error?: string })?.error ?? 'Unknown error'
