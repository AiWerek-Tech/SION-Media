...
_Fokus: Mengubah struktur SQLite lama agar mendukung Multi-Hymnal melalui wipe-out dan re-seeding._
...
_Fokus: Menyesuaikan fungsi-fungsi eksekusi database di Electron Main Process._
...
2. **Song Operations Update**:
   ...
   - Ubah `searchSongs()` agar query me-return juga _hymnal code/name_ hasil JOIN dengan tabel `hymnals` (penting untuk fitur Global Search Engine).
   - Update validasi payload di `addSong()` dan `updateSong()` untuk menerima _schema_ baru.
...
_Fokus: Membuka jalur komunikasi antara Backend (SQLite) dan Frontend (React)._

**Aksi Eksekusi (`src/main/index.ts`):**

1. Daftarkan IPC channels baru untuk CRUD Hymnals (contoh: `get-hymnals`).
2. Sesuaikan _return type_ dari IPC `get-songs` dan `search-songs` yang sekarang mengandung relasi hymnal.

### PHASE 4: State Management & UI Adaptation (Front-End)

_Fokus: Mengadaptasi tampilan ke Arsitektur Multi-Mode._
...
2. **Mode 1 (Library Mode)**:
   - **Hymnal Sidebar**: UI Panel kiri untuk berpindah antar _Hymnal_.
