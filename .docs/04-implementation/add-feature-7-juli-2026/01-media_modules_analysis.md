...
1. **Range-Request Protocol (Streaming Lokal)**:
   Penanganan protokol `local-media://` di main process mendukung *chunked range-request* (HTTP 206). Ini sangat krusial untuk:
   ...
   - Mendukung pencarian posisi durasi (*seeking*) secara instan tanpa lag.
...
3. **OBS-Style Audio Routing**:
   ...
   - **Operator Screen (Muted)**: Video pada panel monitor operator dinonaktifkan suaranya (`muted={true}`) untuk menghindari *echoing* (gema ganda) di dekat komputer operator.
