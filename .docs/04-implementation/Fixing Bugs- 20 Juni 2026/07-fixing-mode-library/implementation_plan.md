...
- Perbarui interface `BiblePackAPI` untuk menyertakan fungsi catatan:
  ```typescript
  getNote: (bookCode: string, chapter: number, verse: number) =>
  ...
  getNotesForChapter: (bookCode: string, chapter: number) =>
    Promise<Array<{ verse: number; note_text: string; highlight_color: string }>>
  ```
