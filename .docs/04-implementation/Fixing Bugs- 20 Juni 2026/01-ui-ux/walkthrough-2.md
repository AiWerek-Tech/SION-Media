...
Kami telah merombak alur pemuatan awal (_booting_) aplikasi untuk menyatukan dua layar splash screen menjadi **satu splash screen native yang mulus** serta menyembunyikan detail pemuatan non-kritis agar berjalan sepenuhnya di latar belakang.

### A. Sinkronisasi Pemanggilan notifyShellReady (App.tsx)

- Mengubah _event hook_ yang mengirim sinyal kesiapan jendela utama ke Electron. Jendela utama baru akan diproyeksikan dan dibuka setelah fase boot mencapai `'background'`, `'ready'`, atau `'failed'`.
- Ini mencegah splash screen native menutup terlalu dini sebelum data aplikasi siap di memori.

### B. Sinkronisasi Tugas Pemuatan Opsional (useAppBootstrap.ts)

- Mengubah proses asinkron untuk menunggu data perpustakaan lagu (`loadSongs`), rundown (`loadPlaylists`), dan nyanyian jemaat (`loadHymnals`) selesai dimuat menggunakan `await Promise.all(optionalTasks)` sebelum berpindah ke fase latar belakang (`'background'`).
- Ini menjamin tidak ada layar kosong atau kedipan saat jendela aplikasi pertama kali muncul di layar pengguna.

### C. Pembersihan Tampilan Layar Splash React (SplashScreen.tsx)

- Mengecualikan fase `'background'` dari daftar fase aktif overlay splash screen React.
- Begitu jendela aplikasi muncul, overlay loading React langsung memudar (_fade-out_), memberikan pengalaman buka aplikasi yang sangat instan dan bersih.

### D. Pemuatan Latar Belakang untuk Tugas Lambat

- Skenario lambat seperti pengecekan pembaruan sistem lewat jaringan internet, pemanasan database Alkitab, dan pengindeksan file media baru, dialihkan sepenuhnya ke latar belakang tanpa memblokir operator untuk mulai berinteraksi dengan aplikasi.
