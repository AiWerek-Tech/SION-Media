...
**Note:** Original files in `.dev-docs/05-guides/` are preserved for backward compatibility. The files in `10-enterprise-refactor-system/` subdirectories are the canonical, reorganized references. They currently point to the originals via README.md files. Physical copies will be created if the originals are moved.
