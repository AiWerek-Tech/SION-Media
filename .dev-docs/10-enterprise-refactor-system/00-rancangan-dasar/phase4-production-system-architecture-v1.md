...
| RT-013 | StageDisplay confidence render     | ??     | Phase 4  | Low    | High           |
| RT-014 | Auto session save (debounced)      | ?     | Phase 4  | Low    | High           |
| RT-015 | MediaEngine LRU cache              | ?     | Phase 4  | Medium | Medium         |
| RT-016 | Atmosphere resolution pipeline     | ??     | Phase 4  | Medium | Medium         |
...
| -------------------------- | -------------- | --------------------------- | -------- | ---- |
| `display_get-all`          | ?? Naming      | Add alias `display:get-all` | Phase 1  | Low  |
...
| All `ipcMain.handle` reads | ?? Unvalidated | Wrap in safeIpcHandle       | Phase 10 | Low  |
...
| ----------------------------- | ----------- | -------- | -------- |
| aria-label on icon buttons    | ?? Partial  | ??       | Phase 5  |
| Focus ring on all interactive | ?? Partial  | ??       | Phase 5  |
...
Location: .dev-docs/06-history/adr-[number]-[title].md
...
5. If architecture decision made:
   ? Create ADR in .dev-docs/06-history/
```
...
Location: .dev-docs/04-implementation/[N]-log-impl-[feature].md
...
? RT-012: StageDisplay confidence render ?? Phase 4
...
? PROJ-009: Stage display sync ?? (legacy only)
